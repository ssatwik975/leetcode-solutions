...
public:
    ...
    bool exist(vector<vector<char>>& board, string word) {
        ...
        vector<vector<bool>> visited(
            n, vector<bool>(m, false)); // Array to keep track of visited cells

        // Convert the word into a character array
        vector<char> wordChar(word.begin(), word.end());

        // Quick check: If the length of the word exceeds the total number of
        // cells on the board, it can't exist
        ...
        // frequency counts to optimize search
        ...
        for (char c : wordChar) {
            if (--counts[c] < 0)
                return false; // If there are more occurrences of a character in
                              // the word than on the board, return false
        }

        // Iterate through each cell in the board and start searching for the
        // word
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (visit(board, wordChar, 0, i, j, n, m, visited))
                    return true; // If the word is found starting from this
                                 // cell, return true
            }
        }
        return false; // If the loop completes without finding the word, return
                      // false
    }

    // Helper function to recursively search for the word starting from a given
    // cell
    bool visit(vector<vector<char>>& board, vector<char>& word, int start,
               int x, int y, int n, int m, vector<vector<bool>>& visited) {
        // Base case: If all characters in the word are found, return true
        if (start == word.size())
            return true;

        // Check for out-of-bounds, already visited cells, and character
        // mismatch
        if (x < 0 || x >= n || y < 0 || y >= m || visited[x][y])
            return false;

        // If the current character in the word does not match the character on
        ...
        return found; // Return whether the word was found starting from the
                      // current cell
    }
